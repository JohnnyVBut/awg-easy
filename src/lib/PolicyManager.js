...
class PolicyManager {
   ...
   *
   * Проходит по enabled правилам в порядке приоритета.
   * Для каждого правила проверяет совпадение source и destination.
   * Возвращает первое совпавшее правило (или null).
   *
   * @param {string} srcIP - source IP пакета
   * @param {string} dstIP - destination IP пакета
   ...
   *
   * @param {object} ep  - endpoint объект { type, aliasId?, value?, invert }
   * @param {string} ip  - IP-адрес для проверки
   * @returns {Promise<boolean>}
   */
  async _matchEndpoint(ep, ip) {
    if (ep.type === 'any') return true;
   ...
   * `ipset test <name> <ip>` → exit 0 если есть, non-zero если нет.
   *
  ...
  async _ipsetTest(setName, ip) {
    try {
      ...
      return false; // не в сете или сет не существует
    }
  }

  /**
   * Проверить, входит ли IPv4-адрес в CIDR-диапазон.
   * Поддерживает: "1.2.3.4/24", "1.2.3.4/32", "1.2.3.4" (одиночный хост).
   *
   * @param {string} ip
   * @param {string} cidr
   * @returns {boolean}
  ...
  /**
   * Преобразовать IPv4-адрес в uint32.
   * @param {string} ip
   * @returns {number}
   */
  _ipToInt(ip) {
