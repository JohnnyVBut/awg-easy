...
const PrefixFetcher = require('./PrefixFetcher');
...
class IpsetManager {
  ...
  async _runGeneratorAsync(jobId, name, opts) {
    ...
    debug(`Job ${jobId}: fetching prefixes via PrefixFetcher (country=${opts.country} asn=${opts.asn} asnList=${opts.asnList})`);

    let prefixes;
    if (opts.asnList) {
      prefixes = await PrefixFetcher.fetchAsnList(opts.asnList);
    } else if (opts.asn) {
      prefixes = await PrefixFetcher.fetchAsn(opts.asn);
    } else if (opts.country) {
      prefixes = await PrefixFetcher.fetchCountry(opts.country);
    } else {
      throw new Error('Generator opts must include country, asn, or asnList');
    }

    if (!prefixes.length) throw new Error('No prefixes returned — check source params');

    // Write to temp file then load into ipset atomically
    await PrefixFetcher.writeToFile(prefixes, outFile);
