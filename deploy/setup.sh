#!/usr/bin/env bash
# =============================================================================
# WireSteer — Full deployment script
...
REBOOT_FLAG="/var/lib/.wiresteer-needs-reboot"
...
save_env() {
  ...
  cat > "$ENV_FILE" << EOF
# WireSteer deploy config — generated by setup.sh
WG_HOST=${WG_HOST:-}
ADMIN_PATH=${ADMIN_PATH:-}
ACME_EMAIL=${ACME_EMAIL:-}
WIRESTEER_PORT=${WIRESTEER_PORT:-8888}
...
echo ""
echo -e "${B}╔══════════════════════════════════════╗${N}"
echo -e "${B}║     WireSteer — Deploy Setup         ║${N}"
echo -e "${B}╚══════════════════════════════════════╝${N}"
echo ""
...
SYSCTL_FILE="/etc/sysctl.d/99-wiresteer.conf"
...
# ═══════════════════════════════════════════════════════════════════════════════
echo ""
echo -e "${B}── Step 5: Build WireSteer${N}"
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 7 — Start WireSteer
# ═══════════════════════════════════════════════════════════════════════════════
echo ""
echo -e "${B}── Step 7: WireSteer${N}"
...
sed -i "s|PORT=.*|PORT=${WIRESTEER_PORT}|"        "$COMPOSE_FILE"
...
for i in $(seq 1 30); do
  STATUS=$(curl -s --max-time 2 "http://127.0.0.1:${WIRESTEER_PORT}/api/health" 2>/dev/null | grep -o '"ok"' || true)
  if [[ "$STATUS" == '"ok"' ]]; then
    ok "WireSteer healthy on 127.0.0.1:${WIRESTEER_PORT}"
    break
  fi
  sleep 1
  [[ $i -eq 30 ]] && fail "WireSteer did not become healthy after 30s — check: docker logs awg-router"
done
...
echo ""
echo -e "${G}╔══════════════════════════════════════════════════════╗${N}"
echo -e "${G}║              WireSteer — Setup Complete              ║${N}"
echo -e "${G}╚══════════════════════════════════════════════════════╝${N}"
echo ""
