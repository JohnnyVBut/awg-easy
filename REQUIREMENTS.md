...
- Заполняются двумя способами:
  1. **Выбор профиля** — dropdown "Select obfuscation profile" из списка сохранённых профилей (Settings → AWG2 Profiles). При выборе все поля заполняются автоматически, H1-H4 рандомизируются заново
...
| Jmax | number | |
| S1, S2, S3, S4 | number | |
| H1, H2, H3, H4 | string | Зоны/диапазоны. При применении к интерфейсу рандомизируются заново |
...
- При выборе профиля поля AWG2 параметров заполняются из профиля (H1-H4 рандомизируются заново)
- После применения профиля параметры можно редактировать вручную
