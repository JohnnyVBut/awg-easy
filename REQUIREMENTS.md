...
|----------|-------|------------|
| Name | ✅ | Произвольное имя |
| Address | ✅ | IP пира в CIDR. Ручной ввод с валидацией (в подсети интерфейса, не занят) |
| Public key | ✅ | Публичный ключ удалённой стороны. Ручной ввод |
| Pre-shared key | ☐ | Опциональный. Кнопка Generate или ручной ввод |
| Endpoint | ☐ | IP:port удалённой стороны. Опциональный |
| PersistentKeepalive | ☐ | Секунды. Опциональный |
| AllowedIPs | ✅ | Подсети, маршрутизируемые через этот пир. Ручной ввод |

**Примечание:** для Interconnect пира приватный ключ **не хранится** на сервере — удалённая сторона управляет своими ключами самостоятельно. QR и Download config недоступны.
...
### 3.3 Формат JSON файла конфигурации Interconnect пира
...
### 3.5 Данные активности (polling из wg show)
...
### 3.6 QR-код и Download config (только для Client пиров)
