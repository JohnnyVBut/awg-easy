...
Интерфейс при создании получает тип, который определяет его назначение:

| Тип | Назначение | Особенности |
|-----|-----------|-------------|
| **Client** | Подключение клиентских устройств (телефоны, ноутбуки) | NAT включён по умолчанию, маршруты добавляются автоматически |
| **Site-to-Site** | Соединение с подобными системами: P2P или P2MP с кастомным роутингом | Disable Routes включён по умолчанию, NAT отключён по умолчанию |

Тип интерфейса влияет на дефолтные значения параметров при создании, но не ограничивает ручную настройку.
...
- Заполняются двумя способами (взаимоисключающие):
  1. **Вручную** — пользователь вводит каждое поле самостоятельно
  2. **Импорт из JSON** — кнопка "Import from JSON", открывает диалог выбора файла, заполняет все поля из файла
- Кнопка "Use Defaults" **удалена**
- После заполнения (любым способом) параметры можно редактировать вручную
...
- Не отмечен (дефолт для Client) → `Table` не пишется в конфиг (wg-quick использует `auto`), NAT правила создаются
- Отмечен (дефолт для Site-to-Site) → `Table = off` в конфиге **и** PostUp/PostDown не содержат MASQUERADE правил (только FORWARD ACCEPT остаётся)

Назначение: для S2S соединений где маршрутизация управляется вручную (PBR). WireGuard по-прежнему использует AllowedIPs для крипторутинга, в системную таблицу маршрутов ничего не добавляет, трансляция адресов не выполняется.
